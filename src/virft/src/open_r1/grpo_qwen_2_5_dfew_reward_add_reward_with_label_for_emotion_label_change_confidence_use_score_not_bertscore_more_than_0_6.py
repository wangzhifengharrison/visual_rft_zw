...
scorer = BERTScorer(
    ...
    lang="en",
    # rescale_with_baseline=True, // 用了这个之后跑出来的结果是0.04....删了之后是0.6....
    device=device
)
...
def reasoninng_attribute_match_result_in_think(ground_truth_label, student_think = ''):
    # ground_truth_label is predicted label of student answer, e.g. 'happy', 'sad', 'fear', 'surprise', 'angry', 'disgust', 'neutral'
    in_context = 0
    in_body_language = 0
    in_emotion = 0
    ...
    e_high_f1 = e_F1[e_F1 > threshold]
    # print('221', e_F1, e_high_f1)
    # print('222', e_high_f1.numel())
    if e_high_f1.numel() > 0:
        # average of all F1 scores > 0.6
        # print('225 come')
        e_avg_high_f1 = e_high_f1.mean().item()
        # print(f'Avg e F1 (above {threshold}):', e_avg_high_f1)
        in_emotion = e_avg_high_f1
    else:
        # print(f'No e F1 scores above {threshold}')
        in_emotion = 0.05
    ...
    c_high_f1 = c_F1[c_F1 > threshold]
    # print('242', c_F1, c_high_f1, c_high_f1.numel())

    if c_high_f1.numel() > 0:
        # average of all F1 scores > 0.6
        # print('248 come')
        c_avg_high_f1 = c_high_f1.mean().item()
        # print(f'Avg c F1 (above {threshold}):', c_avg_high_f1)
        in_context = c_avg_high_f1
    else:
        # print(f'No c F1 scores above {threshold}')
        in_context = 0.05
    ...
    b_high_f1 = b_F1[b_F1 > threshold]
    # print('265', b_F1, b_high_f1, b_high_f1.numel())

    if b_high_f1.numel() > 0:
        # average of all F1 scores > 0.6
        # print('272 come')
        b_avg_high_f1 = b_high_f1.mean().item()
        # print(f'Avg b F1 (above {threshold}):', b_avg_high_f1)
        in_body_language = b_avg_high_f1
    else:
        # print(f'No b F1 scores above {threshold}')
        in_body_language = 0.05
    
    # print('276', in_context, in_body_language, in_emotion)
...
def compute_reward_label(label_results, len_gt):
    # label_results: List of tuples, where each tuple contains (iou, label, confidence) for maximum iou
    label_reward = 0.0
    confidence_reward = 0.0
